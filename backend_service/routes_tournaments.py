...
# Valid state machine transitions (spec: explicit & validated)
VALID_TRANSITIONS = {
...
def update_tournament_status(
    ...
    """
    Enforces the spec state machine:
      draft → active → completed  (no skipping, no reversing)
    """
    tournament = db.query(Tournament).filter(Tournament.id == tournament_id).first()
    if not tournament:
        raise HTTPException(status_code=404, detail="Tournament not found")
    ...
    if body.status not in allowed:
        raise HTTPException(
            status_code=400,
            detail=f"Cannot transition from '{tournament.status}' to '{body.status}'. "
...
def delete_tournament(
    ...
    if tournament.status != TournamentStatus.draft:
        raise HTTPException(status_code=400, detail="Only draft tournaments can be deleted")
